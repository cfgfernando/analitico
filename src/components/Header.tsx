...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-30 bg-slate-900 border-b border-slate-800 text-white shadow-md">
      <div className="w-full px-3 sm:px-4 lg:px-6 flex items-center justify-between h-14 gap-3">
        {/* Left: Sidebar Toggle + Tenant Info & Breadcrumb com Alta Legibilidade */}
        <div className="flex items-center gap-3 min-w-0">
          <button
            onClick={onToggleSidebar}
            className="p-1.5 rounded-sm bg-slate-800 hover:bg-slate-700 text-white border border-slate-700 transition cursor-pointer shrink-0 shadow-xs"
          ...
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="w-7 h-7 rounded-sm bg-emerald-500/20 border border-emerald-500/40 flex items-center justify-center text-emerald-300 shrink-0 font-bold">
              <Landmark className="w-4 h-4" />
            </div>

            <div className="flex flex-col min-w-0">
              <div className="flex items-center gap-2">
                <span className="font-bold text-[13px] font-mono tracking-tight text-white uppercase truncate">
                  {tenantInfo.cidade} / {tenantInfo.uf}
                </span>
                <span className="hidden sm:inline-block text-[10px] font-mono font-bold px-2 py-0.5 rounded-xs bg-slate-800 text-slate-200 border border-slate-700">
        ...
        <div className="flex items-center gap-2.5 shrink-0">
          ...
          {/* SICONFI Live Status */}
          <div
            className="hidden md:flex items-center gap-2 px-3 py-1 rounded-sm bg-slate-950 border border-slate-800 text-xs font-mono text-slate-200"
          ...
          <button
            ...
            className="hidden sm:flex items-center gap-1.5 px-3 py-1 text-xs font-mono font-bold uppercase tracking-wider text-emerald-300 bg-emerald-950/60 border border-emerald-700 rounded-sm hover:bg-emerald-900/80 transition cursor-pointer shadow-xs"
          ...
          <div className="relative" ref={roleDropdownRef}>
            ...
            {roleDropdownOpen && (
              <div className="absolute right-0 mt-1.5 w-60 bg-slate-900 border border-slate-700 rounded-sm shadow-2xl py-1 z-50 animate-in fade-in zoom-in-95 duration-150">
                <div className="px-3 py-1.5 border-b border-slate-800 text-[11px] font-mono font-bold text-slate-300 uppercase">
                  Alternar Perfil de Acesso
                </div>

                <button
                  onClick={() => handleSelectRole('EMPRESA_MASTER')}
                  className={`w-full text-left px-3 py-2.5 text-xs font-mono font-medium flex items-center justify-between hover:bg-slate-800 transition cursor-pointer ${
