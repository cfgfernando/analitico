      ...
      <div className="bg-white dark:bg-navy-950 border border-slate-200/90 dark:border-navy-800/80 rounded-sm p-4 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 font-sans">
        ...
        <div className="flex items-center gap-4 shrink-0 font-sans">
          <div className="text-right">
            <span className="text-[10px] font-bold text-slate-400 uppercase block">Autonomia Fiscal</span>
            <span className="inline-flex items-center gap-1 text-sm font-bold text-slate-900 dark:text-white">
              <Trophy className="w-3.5 h-3.5 text-amber-500" /> {ativo.autonomiaRankingPosicao || payload.grupoComparativo.posicaoAtivo}º Lugar ({payload.grupoComparativo.totalMunicipios} pares)
            </span>
          </div>
          <div className="text-right border-l border-slate-200 dark:border-navy-800 pl-4">
            ...
            <span className="text-base font-extrabold text-emerald-600 dark:text-emerald-400 tabular-nums">{ativo.scoreEficienciaFiscal} / 100</span>
          </div>
        </div>
      </div>

      {/* Frase Executiva em Destaque */}
      <div className="bg-slate-50 dark:bg-navy-900/60 border border-slate-200/90 dark:border-navy-800/80 rounded-sm p-3.5 flex items-center justify-between text-xs font-sans">
        <div className="flex items-center gap-2">
          <Sparkles className="w-4 h-4 text-amber-500 shrink-0" />
          <span className="text-slate-700 dark:text-slate-300">
            <strong className="text-slate-950 dark:text-white">{cidade}</strong> gasta <strong className="text-amber-600 dark:text-amber-400">{ativo.despesaPessoalPct.toFixed(1)}% da RCL</strong> com pessoal; municípios similares da mesma faixa populacional gastam em média <strong className="text-emerald-600 dark:text-emerald-400">{payload.grupoComparativo.mediaDespesaPessoalPct}%</strong>.
          </span>
        </div>
